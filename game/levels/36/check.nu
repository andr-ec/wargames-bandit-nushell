#!/usr/bin/env nu
# Level 36 validation
# Test that bandit36 has correct password file

export def "main check" [expected_password: string] {
    try {
        # Check bandit_pass directory exists
        let bandit_pass_path = "/home/bandit36/bandit_pass"
        if not ($bandit_pass_path | path exists) {
            return { success: false, message: "bandit36 password file not found" }
        }
        ...
        if $current_password != $expected_password {
            return { success: false, message: $"bandit36 password mismatch: got '$current_password', expected '$expected_password'" }
        }
        ...
        # Check file is owned by bandit36
        let file_owner = (ls $bandit_pass_path | get Owner)
        if not ($file_owner | str contains "bandit36") {
            return { success: false, message: "File not owned by bandit36" }
        }

        return { success: true, message: "Level 36 setup complete" }
    } catch {
