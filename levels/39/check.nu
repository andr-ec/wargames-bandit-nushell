#!/usr/bin/env nu
# Level 39 validation
# Test that bandit39 has correct password file

export def "main check" [expected_password: string] -> record {
    try {
        # Check bandit_pass directory exists
        let bandit_pass_path = "/home/bandit39/bandit_pass"
        if not ($bandit_pass_path | path exists) {
            return { success: false, message: "bandit39 password file not found" }
        }
        ...
        if $current_password != $expected_password {
            return { success: false, message: $"bandit39 password mismatch: got '$current_password', expected '$expected_password'" }
        }
        ...
        # Check file is owned by bandit39
        let file_owner = (ls $bandit_pass_path | get Owner)
        if not ($file_owner | str contains "bandit39") {
            return { success: false, message: "File not owned by bandit39" }
        }

        return { success: true, message: "Level 39 setup complete - Congratulations!" }
    } catch {
