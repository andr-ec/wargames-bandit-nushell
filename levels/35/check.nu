#!/usr/bin/env nu
# Level 35 validation
# Test that bandit35 has correct password file

export def "main check" [expected_password: string] -> record {
    try {
        # Check bandit_pass directory exists
        let bandit_pass_path = "/home/bandit35/bandit_pass"
        if not ($bandit_pass_path | path exists) {
            return { success: false, message: "bandit35 password file not found" }
        }
        ...
        if $current_password != $expected_password {
            return { success: false, message: $"bandit35 password mismatch: got '$current_password', expected '$expected_password'" }
        }
        ...
        # Check file is owned by bandit35
        let file_owner = (ls $bandit_pass_path | get Owner)
        if not ($file_owner | str contains "bandit35") {
            return { success: false, message: "File not owned by bandit35" }
        }

        return { success: true, message: "Level 35 setup complete" }
    } catch {
